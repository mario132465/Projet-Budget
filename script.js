 document.addEventListener("DOMContentLoaded", () => {
      ...
      const [pRev, pDep, pSolde] = document.querySelectorAll(".card p");
      ...
      deps.forEach((d, i) => {
        const row = `<tr><td>${d.titre}</td><td>${d.montant.toLocaleString()} F CFA</td><td><button onclick=\"supprimer('depenses', ${i})\">Supprimer</button></td></tr>`;
        tDeps.insertAdjacentHTML("beforeend", row);
      });

      revs.forEach((r, i) => {
        const row = `<tr><td>${r.titre}</td><td>${r.montant.toLocaleString()} F CFA</td><td><button onclick=\"supprimer('revenus', ${i})\">Supprimer</button></td></tr>`;
